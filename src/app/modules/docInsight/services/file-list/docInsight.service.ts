...
export class DocInsightService {
  apiBaseUrl = '';
  ...
  async getResponse(message: string, collection_id: number, thread_id: string) {
    ...
    fetchEventSource(`${this.apiBaseUrl}/ask-doc/chat`, {
      openWhenHidden: true,
      method: 'POST',
      headers: {
  ...
  getImprovedSearch(collection_id: number, faq_id: number) {
    if (!this.ctrl) {
      this.ctrl = new AbortController();
    }
    ...
    //   this.userAuthSer.getOrgId()!
    // );
    // return this.http.post(`${this.apiBaseUrl}/doc-chat/faq/update`, body, { headers });
  }
